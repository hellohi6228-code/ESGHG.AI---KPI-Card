...
import { Settings, LogOut, Activity, DollarSign, Clock, ShieldCheck, BookOpen, Factory, Palette, Edit2, Check, X, TrendingUp, BarChart, LineChart, MessageSquare, Cpu } from 'lucide-react';
...
export default function Dashboard({ 
  ...
  const handleTimeframeChange = (cardId: string, tf: Timeframe) => {
    setTimeframes(prev => ({ ...prev, [cardId]: tf }));
  };
  ...
  // Mock Data Generator based on timeframe and type
  const getMockData = (type: KPIType | null, tf: Timeframe) => {
    const multiplier = tf === 'daily' ? 1 : tf === 'weekly' ? 7 : 30;
    
    if (type === 'price') return { val: `$${(1240.50 * (1 + Math.random()*0.1)).toFixed(2)}`, trend: '+2.4%' };
    if (type === 'time') return { val: `${(94.2 - (multiplier * 0.1)).toFixed(1)}%`, trend: '+1.1%' };
    if (type === 'control') return { val: `${(0.02 * multiplier).toFixed(2)}%`, trend: '-0.01%' };
    if (type === 'training') return { val: `${Math.min(100, 85 + multiplier)}%`, trend: '+5%' };
    if (type === 'emission') return { val: `${(12.4 * multiplier).toFixed(1)}t`, trend: '-2.1%' };
    if (type === 'roi') return { val: `${(15.4 + Math.random()*2).toFixed(1)}%`, trend: '+1.2%' };
    if (type === 'irr') return { val: `${(12.1 + Math.random()).toFixed(1)}%`, trend: '+0.5%' };
    if (type === 'beta') return { val: `${(1.15 + Math.random()*0.1).toFixed(2)}`, trend: '-0.05' };
    return { val: '--', trend: '' };
  };
    ...
    <div className="h-screen w-full flex flex-col overflow-hidden font-sans">
        ...
        <div className="flex items-center gap-2 md:gap-4 relative">
          ...
          <div className="relative">
            <button 
              ...
              className="p-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-base)] rounded-lg transition-colors" 
              title="Change Theme"
            >
          ...
          <button onClick={onReset} className="p-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-base)] rounded-lg transition-colors" title="Reconfigure Dashboard">
      ...
      <main className="flex-1 p-2 md:p-4 overflow-y-auto md:overflow-hidden">
        <div className={`grid gap-2 md:gap-4 h-full w-full ${getGridClass(cards.length)}`}>
          {cards.map((card) => {
            const Icon = card.type ? ICONS[card.type] : Activity;
            const tf = timeframes[card.id] || 'daily';
            const { val: mockValue, trend: mockTrend } = getMockData(card.type, tf);
            const { val: secValue } = getMockData(card.type, tf === 'daily' ? 'weekly' : 'monthly');
            
            const isPositive = (mockTrend.startsWith('+') && card.type !== 'emission' && card.type !== 'price' && card.type !== 'control') || 
                               (mockTrend.startsWith('-') && (card.type === 'emission' || card.type === 'control'));
                  ...
                  <>
                    ...
                    <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                      <button 
                        ...
                        title="Send to xox for labeling"
                      >
                    ...
                    <div className="absolute bottom-3 right-3 flex bg-[var(--bg-base)] border border-[var(--border-subtle)] rounded-lg p-0.5 z-20">
                      {(['daily', 'weekly', 'monthly'] as Timeframe[]).map(t => (
                        <button
                          key={t}
                          onClick={() => handleTimeframeChange(card.id, t)}
                    ...
                    {card.layout === 'hero-top-left' ? (
                      <div className="flex flex-col h-full justify-between z-10">
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="text-sm font-bold text-[var(--text-primary)] mb-1 truncate max-w-[200px]">{card.title || 'Untitled Card'}</h3>
                            <div className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider mb-1">{card.indicator || 'Unconfigured'}</div>
                            <div className="text-4xl md:text-6xl font-mono tracking-tighter text-[var(--text-primary)]">{mockValue}</div>
                          </div>
                      ...
                      <div className="flex flex-col h-full z-10">
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          <div>
                            <h3 className="text-sm font-bold text-[var(--text-primary)]">{card.title || 'Untitled Card'}</h3>
                        ...
                        <div className="flex-1 grid grid-cols-2 gap-4 items-center pb-6">
                          <div>
                            ...
                            <div className="text-2xl md:text-3xl font-mono text-[var(--text-primary)]">{mockValue}</div>
                      ...
                      <div className="flex flex-col h-full z-10">
                        <div className="flex items-start justify-between mb-2 md:mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="text-sm font-bold text-[var(--text-primary)] truncate max-w-[140px] md:max-w-[200px]">{card.title || 'Untitled Card'}</h3>
                        ...
                        <div className="flex-1 flex flex-col justify-center">
                          <div className="flex items-baseline gap-3">
                            <span className="text-3xl md:text-5xl font-mono tracking-tight text-[var(--text-primary)]">{mockValue}</span>
                            {mockTrend && (
